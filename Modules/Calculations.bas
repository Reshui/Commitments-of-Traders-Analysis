...
Public Function Disaggregated_Multi_Calculations(ByRef inputA() As Variant, weeksToCalculateCount As Long, ByVal producerNetColumn As Byte, Time1 As Long, Time2 As Long) As Variant()
    ...
    Dim iceContractCodes() As String, contractCodeColumn As Byte, rowIndex As Long, outputA() As Variant, _
    iCount As Byte, openInterest As Long, Start As Long, Finish As Long, INTE_B() As Byte, Z As Long, columnIndexByte As Byte
    ...
    For rowIndex = Start To Finish
        ...
        For iCount = 0 To 3 'Producer Net , Swap Net , Managed Net , Other Net
            ...
            inputA(rowIndex, producerNetColumn + iCount) = inputA(rowIndex, columnIndexByte) - inputA(rowIndex, columnIndexByte + 1)
        Next iCount
        
        inputA(rowIndex, producerNetColumn + 4) = inputA(rowIndex, producerNetColumn) + inputA(rowIndex, producerNetColumn + 1) 'Commercial Net
        
        If rowIndex >= 2 Then
            For iCount = 0 To 2 'Producer Net Change,Swap Net Change,Commercial Net Change
                columnIndexByte = Array(0, 1, 4)(iCount)
                inputA(rowIndex, producerNetColumn + 20 + iCount) = inputA(rowIndex, producerNetColumn + columnIndexByte) - inputA(rowIndex - 1, producerNetColumn + columnIndexByte)
            Next iCount
        
            For iCount = 6 To 7 'Commercial Long/Short Change
                inputA(rowIndex, producerNetColumn + 17 + iCount) = (inputA(rowIndex, iCount - 2) + inputA(rowIndex, iCount)) - (inputA(rowIndex - 1, iCount - 2) + inputA(rowIndex - 1, iCount))
            Next iCount
        End If
        
        openInterest = inputA(rowIndex, 3)
        
        If inputA(iRow, 3) <> 0 Then
            'Producer/OI
            inputA(rowIndex, producerNetColumn + 11) = inputA(rowIndex, producerNetColumn) / openInterest
            'Swap/OI
            inputA(rowIndex, producerNetColumn + 12) = inputA(rowIndex, producerNetColumn + 1) / openInterest
        End If
        'Commercial/OI
        inputA(rowIndex, producerNetColumn + 10) = inputA(rowIndex, producerNetColumn + 11) + inputA(rowIndex, producerNetColumn + 12)
        
    Next rowIndex
    
    If Not IsError(Application.Match(inputA(UBound(inputA, 1), contractCodeColumn), iceContractCodes, 0)) Then
            ...
            If rowIndex > 1 Then
                For columnIndexByte = 3 To 18
                    Select Case columnIndexByte
                        Case 15, 16
                        Case Else
                            inputA(rowIndex, columnIndexByte + 16) = inputA(rowIndex, columnIndexByte) - inputA(rowIndex - 1, columnIndexByte)
                    End Select
                Next columnIndexByte
            End If
            'Below are the calculations for total reportable positions[OI-Non Reportable for position.]
            inputA(rowIndex, 15) = inputA(rowIndex, 3) - inputA(rowIndex, 17)  'Reportables long
            inputA(rowIndex, 16) = inputA(rowIndex, 3) - inputA(rowIndex, 18)  'Reportables Short
            'Calculate total reportable traders.
            inputA(rowIndex, 63) = inputA(rowIndex, 52) + inputA(rowIndex, 54) + inputA(rowIndex, 57) + inputA(rowIndex, 60)
            inputA(rowIndex, 64) = inputA(rowIndex, 53) + inputA(rowIndex, 55) + inputA(rowIndex, 58) + inputA(rowIndex, 61)
                        
            If rowIndex > 1 Then
                'Calculate change in Reportable positions.
                inputA(rowIndex, 31) = inputA(rowIndex, 15) - inputA(rowIndex - 1, 15)
                inputA(rowIndex, 32) = inputA(rowIndex, 16) - inputA(rowIndex - 1, 16)
            End If
    ...
    If UBound(inputA, 1) > Time1 Then     'Year 3
        ...
        For iCount = 0 To 7
            ...
            For rowIndex = Start To Finish  'From First Missed to Most recent
                inputA(rowIndex, columnIndexByte) = INTE_B(Z)
                Z = Z + 1
            Next rowIndex
    ...
    If UBound(inputA, 1) > Time2 Then   'Month6
        ...
        For iCount = 0 To 2
            ...
            Z = 1
            For rowIndex = Start To Finish          'From First Missed to Most recent
                inputA(rowIndex, columnIndexByte) = INTE_B(Z)  '   xxx/oi
                Z = Z + 1
            Next rowIndex
    ...
    For rowIndex = Start To Finish                  'First Missed to most recent or Last row if Weekl
        If rowIndex > Time1 + 6 Then                ' Movement Index Calculation
            inputA(rowIndex, columnIndexByte) = inputA(rowIndex, iCount) - inputA(rowIndex - 6, iCount)
        End If
    Next rowIndex
    ...
    For rowIndex = Start To Finish
        For iCount = 1 To UBound(inputA, 2)
            outputA(Z, iCount) = inputA(rowIndex, iCount)
        Next iCount
        Z = Z + 1
    Next rowIndex
